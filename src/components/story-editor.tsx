...
import { apiFetch, getCsrfToken } from "@/lib/csrf-client";
...
export function StoryEditor({ storyId }: StoryEditorProps) {
  ...
  const [uploadStatus, setUploadStatus] = useState<string>("not_uploaded");
  ...
  useEffect(() => {
    ...
    fetch(`/api/stories/${storyId}`)
      ...
      .then((data) => {
        ...
        setCoverImagePublicId(data.coverImagePublicId ?? null);
        setUploadStatus(data.uploadStatus || "not_uploaded");
        setPages(
  ...
  const confirmDeletePage = async () => {
    ...
    try {
      const res = await apiFetch(`/api/pages/${deletePageId}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const storyBody = { title, ageGroup, genre, characterGender, tags, uploadStatus, coverImageUrl, coverImagePublicId };
      ...
      if (isEdit) {
        ...
        if (!savedStoryId) throw new Error("Failed to create story: no ID returned");
      }

      // 2. Save pages
      const newPages = pages.filter((p) => p._isNew);
      const dirtyPages = pages.filter((p) => !p._isNew && p._isDirty && p.id);

      if (newPages.length > 0) {
        // Bulk-create sends ALL pages so the server can do a clean replace.
        // This avoids rate-limit issues and handles retries safely.
        ...
        if (!res.ok) {
          ...
          throw new Error(errBody?.error || "Failed to save pages");
        }
      } else {
        // No new pages — patch only the dirty existing ones
        for (const page of dirtyPages) {
          ...
          if (!res.ok) throw new Error(`Failed to update page ${page.pageNumber}`);
        }
      }
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => setUploadStatus(uploadStatus === "uploaded" ? "not_uploaded" : "uploaded")}
                  className={`relative inline-flex h-7 w-12 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:ring-offset-1 ${
                    uploadStatus === "uploaded" ? "bg-emerald-500" : "bg-red-400"
                  }`}
                  role="switch"
                  aria-checked={uploadStatus === "uploaded"}
                >
                  <span
                    className={`inline-block h-5 w-5 transform rounded-full bg-white shadow-sm ring-0 transition-transform duration-200 ${
                      uploadStatus === "uploaded" ? "translate-x-6" : "translate-x-1"
                    }`}
                  />
                </button>
                <Badge
                  variant={uploadStatus === "uploaded" ? "default" : "secondary"}
                  className={
                    uploadStatus === "uploaded"
                      ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300 border-emerald-200/60 dark:border-emerald-800/40"
                      : "bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300 border-red-200/60 dark:border-red-800/40"
                  }
                >
                  {uploadStatus === "uploaded" ? "Uploaded" : "Not Uploaded"}
                </Badge>
              </div>
            </div>
