...
function GenresTab() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const res = await apiFetch("/api/settings/genres", {
        method: "POST",
  ...
  const saveEdit = async (id: string) => {
    try {
      const res = await apiFetch(`/api/settings/genres/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await apiFetch(`/api/settings/genres/${deleteId}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
...
function LookupTab({
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const res = await apiFetch(`/api/settings/${endpoint}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await apiFetch(`/api/settings/${endpoint}/${deleteId}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
        toast.error(data.error || "Failed to delete");
        return;
