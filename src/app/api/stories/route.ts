...
export async function GET(request: NextRequest) {
  try {
    ...
    if (filters.uploadStatus) where.uploadStatus = filters.uploadStatus;
  ...
  try {
    ...
    const story = await prisma.story.create({
      data: {
        ...
        createdById: user.id,
        ...(parsed.data.uploadStatus ? { uploadStatus: parsed.data.uploadStatus } : {}),
        ...(parsed.data.coverImageUrl ? { coverImageUrl: parsed.data.coverImageUrl } : {}),
        ...(parsed.data.coverImagePublicId ? { coverImagePublicId: parsed.data.coverImagePublicId } : {}),
